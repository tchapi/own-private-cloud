...
docker-compose run --rm --entrypoint "\
  mkdir -p /etc/letsencrypt/live/${notes_url} /etc/letsencrypt/live/${passbolt_url} /etc/letsencrypt/live/${cozy_url}" certbot
docker-compose run --rm --entrypoint "\
...
docker-compose run --rm --entrypoint "\
  openssl req -x509 -nodes -newkey rsa:1024 -days 1\
    -keyout '/etc/letsencrypt/live/${cozy_url}/privkey.pem' \
    -out '/etc/letsencrypt/live/${cozy_url}/fullchain.pem' \
    -subj '/CN=localhost'" certbot
...
docker-compose run --rm --entrypoint "\
  certbot certonly --webroot -w /var/www/letsencrypt \
    --email ${email} \
    -d ${cozy_url} -d settings.${cozy_url} -d drive.${cozy_url} -d photos.${cozy_url} -d contacts.${cozy_url} \
    --rsa-key-size 4096 \
    --agree-tos \
    --force-renewal" certbot
